...
<template>
  <div class="group relative bg-white border rounded-lg overflow-hidden hover:shadow-lg transition-shadow duration-300">
    ...
    <div class="p-4">
      ...
      <h3 class="text-lg font-bold text-gray-900 mt-1 group-hover:text-green-600 transition-colors">
        {{ product.name }}
      </h3>
      ...
      <div class="mt-3 flex items-center">
        ...
        <span v-if="product.discount && product.discount > 0" class="ml-2 text-sm text-gray-500 line-through">
      ...
      <div class="mt-2 flex items-center">
        <div class="flex">
          <span v-for="i in 5" :key="i" class="text-yellow-400">
            <svg v-if="i <= Math.round(product.rating)" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
    ...
    <div class="p-4 pt-0">
      <a :href="productUrl" class="block w-full py-2 text-center font-medium text-green-600 bg-white border border-green-600 rounded hover:bg-green-600 hover:text-white transition-colors">
        Voir le produit
      </a>
    </div>
  </div>
